...
//     if (eneg < 1.0) continue;
    ...
    for (size_t ii = 0; ii < ntrck; ++ii) {
      if (det_idv[ii] == -1) {
        primary_pdg = pdgv[ii];
        if (primary_pdg == 11)  lhist->FillHistW("primary_e_electron", 0, enegv[ii], ev_weight);
        if (primary_pdg == 22)  lhist->FillHistW("primary_e_gamma", 0, enegv[ii], ev_weight);
        if (primary_pdg == -11) lhist->FillHistW("primary_e_positron", 0, enegv[ii], ev_weight);
      }
    }
    if (primary_pdg == -99) {
...
//   lhist->DrawHist2D_BT15("tracking_planes_vtxx_vtxy");
  ...
  for_each(pprimet.begin(), pprimet.end(), [=, &lhist](const std::string hhs) {lhist->Scale(hhs, nbx);});
  ...
  for_each(cesige.begin()+1, cesige.end(), [=, &lhist](const std::string hhs) {lhist->Scale(hhs, nbx);});
  ...
  for_each(cesigx.begin(), cesigx.end(), [=, &lhist](const std::string hhs) {lhist->Scale(hhs, nbx);});
